...
import { UserTable } from "@/components/MyTable";
...
const Users = () => {
  const uris = useMemo(() => {
    return ["/users", "/organizations/active"];
  }, []);
  const {
    data: allDatas = [],
    error,
  ...
  const metadatas = allMetadatas?.filter((m) => m.title === "users")[0]
    ?.metadatas;
