...
const QuestionTypes = () => {
  const uris = useMemo(() => {
    return ["/questiontypes", "/modules/active", "/assessments/active"];
  }, []);
  const {
    data: allDatas = [],
    error,
  ...
  const metadatas = allMetadatas?.filter((m) => m.title === "questiontypes")[0]
    ?.metadatas;
  ...
  return (
    <div className="mx-20 flex flex-col justify-center items-start">
      ...
      <p className="my-3 font-bold text-blue-900 text-xl">
        {t("businesses.questiontypes.title")}
      </p>
      {modal.open ? (
        <FullModal
          action={modal.action}
          ...
          url={uris}
          mutate={mutate}
          selection={{ modules: allDatas[1], assessments: allDatas[2] }}
        />
      ) : (
        <Fragment>
